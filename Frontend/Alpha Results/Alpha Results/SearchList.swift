...
struct SearchList: View {
    ...
    @State private var athletes =  Wrapper(id: 0, athletes: [Athlete(tf: nil, xc: nil, name: "", school: "", grade: "")])
        ...
        HStack{
            HStack{
                ...
                TextField("search", text: $searchText, onEditingChanged: { isEditing in
                    ...
                    print("onCommit")
                    self.athletes = search(searchText: self.searchText)
                    self.searched = true
...
private func search(searchText: String) -> Wrapper{
    ...
    // HTTP Request
    let url = URL(string: ngrok + index + searchText)!
    let task = session.dataTask(with: url, completionHandler: { (data: Data?, response: URLResponse?, error: Error?) -> Void in
        // Parse the data in the response and use it
    })
    
    let temp1 = Athlete(tf: nil, xc: nil, name: "Evan Jameson", school: "CPSLO", grade: "12")
    let temp2 = Wrapper(id: 1, athletes: [temp1])
